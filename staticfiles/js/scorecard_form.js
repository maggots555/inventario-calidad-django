...
class ScorecardFormHandler {
    ...
    init() {
        ...
        this.numeroSerieInput.addEventListener('keypress', (event) => {
            if (event.key === 'Enter') {
        ...
        // También buscar cuando presiona Enter
        this.numeroOrdenInput.addEventListener('keypress', (event) => {
            if (event.key === 'Enter') {
                event.preventDefault(); // Evitar que envíe el formulario
                this.buscarOrdenPorNumeroCliente();
            }
     ...
     */
    async buscarOrdenPorSerie() {
        var _a;
        // Validar que hay algo escrito
        const numeroSerie = (_a = this.numeroSerieInput) === null || _a === void 0 ? void 0 : _a.value.trim().toUpperCase();
        if (!numeroSerie) {
            this.limpiarInfo();
            return;
        }
     ...
     */
    async buscarOrdenPorNumeroCliente() {
        var _a;
        // Validar que hay algo escrito
        const numeroOrden = (_a = this.numeroOrdenInput) === null || _a === void 0 ? void 0 : _a.value.trim();
        if (!numeroOrden) {
            this.limpiarInfo();
            return;
        }
     ...
     */
    autocompletarCampos(orden, origen) {
     ...
     */
    mostrarOrdenNoEncontrada(mensaje) {
        if (!this.infoContainer)
            return;
        this.infoContainer.innerHTML = `
     ...
     */
    mostrarCargando(mensaje = 'Buscando orden en el sistema...') {
        if (!this.infoContainer)
            return;
        this.infoContainer.innerHTML = `
