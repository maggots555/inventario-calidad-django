...
(function listaSucursalesMain(): void {
    /** Instancia mínima del Modal de Bootstrap 5 (CDN en base.html). */
    interface BsModalInstance {
        show(): void;
    }

    /** API de Bootstrap que usamos (tipado local para no chocar con otros .ts). */
    interface BsApi {
        Modal: {
     ...
     */
    function obtenerBootstrap(): BsApi {
        return (window as unknown as { bootstrap: BsApi }).bootstrap;
    }
    ...
    function initEscape(): void {
        ...
        document.addEventListener('keydown', (event: KeyboardEvent) => {
            ...
            const valorBusqueda =
                busqueda instanceof HTMLInputElement ? busqueda.value : '';
            const valorEstado =
                estado instanceof HTMLSelectElement ? estado.value : '';
            const valorCiudad =
                ciudad instanceof HTMLSelectElement ? ciudad.value : '';
            // El chip "Con encargado" vive como hidden, no como select.
            const tieneEncargado = Boolean(
                form.querySelector('input[name="con_encargado"]')
            );
     ...
     */
    function abrirModalEliminar(boton: HTMLElement): void {
     ...
     */
    function initModalEliminar(): void {
        document.addEventListener('click', (event: MouseEvent) => {
            ...
            const boton = target.closest<HTMLElement>('.ls-btn-eliminar');
            if (!boton) {
                return;
            }
